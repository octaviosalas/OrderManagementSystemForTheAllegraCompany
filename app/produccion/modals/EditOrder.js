...
export default function EditOrderModal({
  ...
  const [newOrderCost, setNewOrderCost] = useState(
    type === "orders" ? orderData.manufacturingCost : ""
  );
  const [newOrderState, setNewOrderState] = useState(
    type === "orders" ? orderData.state : ""
  );
  const [newUserName, setNewUserName] = useState(
    type === "users" ? userData.name : ""
  );
  const [newUserSurname, setNewUserSurname] = useState(
    type === "users" ? userData.surname : ""
  );
  const [newUserEmail, setNewUserEmail] = useState(
    type === "users" ? userData.email : ""
  );
  const [newUserRol, setNewUserRol] = useState(
    type === "users" ? userData.rol : ""
  );
  ...
  const saveOrderDataChanges = () => {
    ...
    axios
      .put(`http://localhost:4000/editOrder/${orderId}`, newOrderData)
      .then((res) => {
  ...
  const saveUserDataChanges = () => {
    console.log(peticionUsersAgain);
    const newUserData = {
      name: newUserName,
    ...
    axios
      .put(`http://localhost:4000/editUserData/${userId}`, newUserData)
      .then((res) => {
        console.log(res.data);
        setSuccesMessage(true);
        setTimeout(() => {
            ...
            <>
              ...
              <ModalBody>
                <div className="flex flex-col items-center justify-center mt-6">
                  ...
                  {succesMessage ? (
                    <p className="font-bold text-blue-600 text-sm mt-8">
                      {type === "orders"
